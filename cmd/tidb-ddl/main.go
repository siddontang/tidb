...
var (
	...
	statusPort  = flag.Int("status", 10081, "tidb status port")
)
...
func main() {
	...
	logger.Info("Starting TiDB DDL Service",
	...
	if err := manager.Register(ddlService); err != nil {
		logger.Fatal("Failed to register DDL service", zap.Error(err))
	}
	...
	logger.Info("TiDB DDL Service started",
	...
	logger.Info("Shutting down TiDB DDL Service")
	...
	logger.Info("TiDB DDL Service stopped")
}
