...
func main() {
	...
	logger.Info("Starting TiDB Transaction Service",
	...
	if err := manager.Register(txnSvc); err != nil {
		logger.Fatal("Failed to register transaction service", zap.Error(err))
	}
	...
	logger.Info("TiDB Transaction Service started",
	...
	logger.Info("Shutting down TiDB Transaction Service")
	...
	logger.Info("TiDB Transaction Service stopped")
}
